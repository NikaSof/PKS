...
import 'package:pks_pr10/models/cart.dart';
...
class CartApi {
  ...
  Future<List<Cart>> getCart(int userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load cart_bloc');
      }
    } catch (e) {
  ...
  Future<void> addToCart(int userId, int productId, int quantity) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to add item to cart_bloc');
      }
    } catch (e) {
      throw Exception('Error: $e');
    }
  ...
  Future<void> removeFromCart(int userId, int productId) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to remove item from cart_bloc');
      }
    } catch (e) {
      throw Exception('Error: $e');
    }
